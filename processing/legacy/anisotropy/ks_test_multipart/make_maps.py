...
def get_start_stop_rows(n_rows, n_batches, batch_idx):
...
if __name__ == "__main__":
    ...
    # np.random.seed(args.batch_idx)
    # print('random seed = {}'.format(args.batch_idx))
    # print('type(random seed) = {}'.format(type(args.batch_idx)))
    ...
    with pd.HDFStore(df_file, mode='r') as store:
        ...
        start_row, stop_row = get_start_stop_rows(n_rows, args.n_batches,
                                                  args.batch_idx)
        for df in store.select('dataframe', chunksize=args.chunksize,
            ...
            times = get_random_times(store, n_rows, df.shape[0], n_resamples=20)
