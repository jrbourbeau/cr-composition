...
def plot_data_MC_comparison(sim_df, data_df, var_key, var_bins, var_label,
    ...
    for composition in comp_list:
        comp_mask = sim_df['MC_comp'] == composition
        sim_variable = sim_df[comp_mask][var_key]
        sim_weights = sim_df[comp_mask]['weights']
        ...
        data_weights = np.array([1/livetime]*len(data_df[var_key]))
        ax2 = plot_data_MC_ratio(sim_variable, sim_weights,
                           data_df[var_key], data_weights, var_bins,
    ...
    plot_rate(data_df[var_key], data_weights, bins=var_bins,
...
def save_data_MC_plots(config, june_july_only):
    ...
    sim_df = comp.load_dataframe(datatype='sim', config='IC86.2012',
    ...
    weights = get_sim_weights(sim_df)
    ...
    gs_s125 = plot_data_MC_comparison(sim_df, data_df, 'log_s125',
    ...
    gs_dEdX = plot_data_MC_comparison(sim_df, data_df, 'log_dEdX',
    ...
    gs_zenith = plot_data_MC_comparison(sim_df, data_df, 'lap_cos_zenith',
