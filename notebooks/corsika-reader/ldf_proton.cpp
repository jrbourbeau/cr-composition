...
int main() {
  ...
  char cmd[1024] = { "ls /data/sim/IceTop/2009/generated/CORSIKA-ice-top/10410/6.0/DAT*.bz2 | head -100" }; // 1 PeV proton files
  FILE *ls = popen(cmd, "r");
  ...
  while (fscanf(ls, "%s\n", file) == 1) {
    ...
    stringstream ss;
    ss << file_idx;
    string file_idx_str = ss.str();

    string file_name = "rootfiles/ldf_proton_1PeV_";
    file_name.append(file_idx_str);
