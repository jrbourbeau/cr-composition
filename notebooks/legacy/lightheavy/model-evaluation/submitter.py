...
if __name__ == "__main__":
    ...
    # Maximum tree depth
    dagman = add_parameter(dagman, 'max_depth', range(1, 11), 'int',
                    validation_ex, merge_ex, **vars(args))

    # Learning rate
    dagman = add_parameter(dagman, 'learning_rate', np.arange(0.1, 1.1, 0.1), 'float',
                    validation_ex, merge_ex, **vars(args))

    # Number of estimators
    dagman = add_parameter(dagman, 'n_estimators', range(10, 220, 20), 'int',
                    validation_ex, merge_ex, **vars(args))

    if args.pipeline == 'GBDT':
        #Minimum samples in a leaf
        dagman = add_parameter(dagman, 'min_samples_leaf', range(1, 500, 50), 'int',
                        validation_ex, merge_ex, **vars(args))

        # Minimum samples to split
        dagman = add_parameter(dagman, 'min_samples_split', range(1, 500, 50), 'int',
                        validation_ex, merge_ex, **vars(args))
