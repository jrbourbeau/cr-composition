...
def load_dataframe(datatype='sim', config='IT73', return_cut_dict=False):

    # Load simulation dataframe
    mypaths = Paths()
    infile = '{}/{}_{}/{}_dataframe.hdf5'.format(mypaths.comp_data_dir,
    ...
    # Some conbined cuts
    cut_dict['lap_reco_success'] = (cut_dict['lap_fitstatus_ok']) & (cut_dict['lap_beta'])
    ...
    for dist in ['50', '80', '125', '180', '250', '500']:
        df['log_s{}'.format(dist)] = np.log10(df['lap_s{}'.format(dist)])
